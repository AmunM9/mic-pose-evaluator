...
# Subnets públicas — ALB y NAT Gateway
...
# Subnets privadas — ECS Fargate y RDS
...
# NAT Gateway para que ECS Fargate en subnet privada salga a internet (OpenAI API)
...
resource "aws_security_group" "api" {
  ...
  tags = merge(local.tags, { Name = "${local.name}-sg-api" })
}
...
resource "aws_ecs_task_definition" "api" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "DATABASE_URL", value = "postgresql+asyncpg://micpose:${var.db_password}@${aws_db_instance.main.address}:5432/micpose" },
...
resource "aws_cloudfront_distribution" "frontend" {
  ...
  # SPA: redirigir 404 a index.html para que Next.js maneje el routing
